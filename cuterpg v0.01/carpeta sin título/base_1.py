        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if not event.key == pygame.K_ESCAPE:
                    ...
                    if pygame.key.name(event.key)=='up' and not posicion[1]==0 and puede_estar(posicion[0],posicion[1]-1,1):
                        #if not pisable[matriz[posicion[0]][posicion[1]-1]]:
                        posicion[1]-=1
                    if pygame.key.name(event.key)=='down' and not posicion[1]==len(matriz)-1 and puede_estar(posicion[0],posicion[1]+1,1):
                        posicion[1]+=1
                    if pygame.key.name(event.key)=='left' and not posicion[1]==0 and puede_estar(posicion[0]-1,posicion[1],1):
                        posicion[0]-=1
                    if pygame.key.name(event.key)=='right' and not posicion[1]==len(matriz[0])-1 and puede_estar(posicion[0]+1,posicion[1],1):
                        posicion[0]+=1
