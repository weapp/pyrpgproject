...
class Repetition:
    ...
    def new_event(self,event):
        ...
        if event.type == pygame.KEYUP:
            if event.key in (pygame.K_UP, pygame.K_DOWN, pygame.K_LEFT, pygame.K_RIGHT):
                self.events.remove(pygame.key.name(event.key))
                self.cont=0
    ...
    def update(self):
        ...
        if not self.cont:
            if len(self.events):
                if self.events[len(self.events)-1] == pygame.key.name(pygame.K_UP):
                    self.pos[1]-=1
                    pygame.event.post(self.event_up)
                if self.events[len(self.events)-1] == pygame.key.name(pygame.K_DOWN):
                    self.pos[1]+=1
                    pygame.event.post(self.event_down)
                if self.events[len(self.events)-1] == pygame.key.name(pygame.K_LEFT):
                    self.pos[0]-=1
                    pygame.event.post(self.event_left)
                if self.events[len(self.events)-1] == pygame.key.name(pygame.K_RIGHT):
                    self.pos[0]+=1
